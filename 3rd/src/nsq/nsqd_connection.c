...
//主动关闭连接，只触发close回调，不重连
void nsqd_connection_disconnect_active(struct NSQDConnection *conn)
{
    conn->is_active_close = 1;
    buffered_socket_close(conn->bs);
}
